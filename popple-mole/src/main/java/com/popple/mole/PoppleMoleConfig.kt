...
@ConfigGroup("popple-mole")
interface PoppleMoleConfig : Config {
    @ConfigItem(keyName = "startBtn", name = "Start/Stop", description = "Start/stop the script", position = 150)
    fun startButton(): Button? {
        return Button()
    }

    @ConfigItem(keyName = "prayers", name = "1 tick flick quick prayers", description = "1 tick flicks selected quick prayers", position = 0)
